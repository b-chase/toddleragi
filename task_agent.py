...
class TaskAgent:
    ...
    def complete_task(self):
        ...
        for i in range(5):
            ...
            if task_status == 'COMPLETE' and self.result is not None:
                ...
                print(Fore.YELLOW + f'{task_status}: {self.task_name}'+Fore.RESET)
    ...
    def execute_task(self) -> dict:
        ...
        """
        print(f'Agent: Okay, I will get to work on "{self.task_name}"')
        
        prompt = '''You are a task-executing AI and have received this message from your manager:
        {''' + f'''
            "project_objective": "{self.objective}",
            "new_task": "{self.task_name}", 
            {'completed_tasks: '+', '.join(self.context) + ", " if self.context else ""}
            {'manager feedback: ' + self.feedback if self.feedback else ''}
        '''+'''}
